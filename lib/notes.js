 ...
 */
export async function getNotes(groupSlug, pageSlug) {
  const source = readSource(groupSlug, pageSlug);
  ...
  const intro = await compileSegment(introSegment.lines.join("\n"));
  const sections = [];
...
/** Compiles a whole prose page (about, support, home intro) as one unit. */
export async function getProsePage(...segments) {
  const source = readSource(...segments);
  if (source == null) return null;
  return compileSegment(source);
}
