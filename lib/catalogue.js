    ...
    pages: [
      {
        ...
        models: ["AirTag"],
        // Promoted to the home page; hidden from the Others grid and its
        // breadcrumb sits directly under Home (the URL keeps /others/airtag).
        hidden: true,
        topLevel: true,
      },
...
export function getGroupHeroCase(group) {
  // A group can pin its card artwork to a specific SKU with `hero`.
  if (group.hero) {
    const match = rows().find((row) => row.SKU === group.hero);
    if (match) return match;
  }
  for (const page of group.pages) {
