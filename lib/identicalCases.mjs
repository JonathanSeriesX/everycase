...
const CSV_PATH = path.join(process.cwd(), "scripts", "identical.csv");

let cachedGroupsBySku;

// The CSV stores group membership rather than every possible SKU pair. This
// keeps a group of n identical products to n rows instead of n * (n - 1).
function loadGroupsBySku() {
  ...
  for (const row of rows) {
    ...
    if (existingGroup && existingGroup !== groupId) {
      throw new Error(
        `SKU ${sku} belongs to multiple identical-case groups: ${existingGroup}, ${groupId}`,
      );
    }
...
export function getIdenticalSkus(sku) {
